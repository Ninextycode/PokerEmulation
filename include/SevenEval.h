...
public:
  ...
  static inline uint16_t GetRank(int i, int j, int k, int l, int m, int n,
    ...
    int flush_key = 0;
    if (suit[i] == flush_suit) flush_key  = flush_array[i];
    if (suit[j] == flush_suit) flush_key += flush_array[j];
    if (suit[k] == flush_suit) flush_key += flush_array[k];
    if (suit[l] == flush_suit) flush_key += flush_array[l];
    if (suit[m] == flush_suit) flush_key += flush_array[m];
    if (suit[n] == flush_suit) flush_key += flush_array[n];
    if (suit[p] == flush_suit) flush_key += flush_array[p];
